...
export class HaGrowBoxCard extends LitElement implements LovelaceCard {
  ...
  private findRelatedSensor(plantEntityId: string, sensorType: string): string | null {
    ...
    const relatedSensors = Object.keys(this.hass.states).filter(entityId => {
      ...
      if (!isRelatedToPlant) return false;
      
      // Check if it's the right sensor type
      const sensorTypeMatches: { [key: string]: string[] } = {
  ...
  private renderFlowerCardAttribute(sensorType: string, plantEntity: any, plantConfig: any): TemplateResult {
    ...
    if (!plantInfo?.result?.[sensorType]) {
  ...
  private handleSensorClick(plantEntity: any, sensorType: string, sensorEntityId?: string): void {
